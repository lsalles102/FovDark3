...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if request.url.path.endswith('.html') or 'text/html' in response.headers.get('content-type', ''):
            # Obter o domínio atual para CSP dinâmica
            current_host = request.headers.get('host', '')
            current_protocol = 'https' if request.url.scheme == 'https' else 'https'  # Forçar HTTPS
            current_domain = f"{current_protocol}://{current_host}"
