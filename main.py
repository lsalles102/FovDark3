...
async def create_payment_preference(
    ...
    try:
        ...
        preference_id = f"pref_{current_user.id}_{product_id}_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
        ...
        new_payment = Payment(
            ...
            status="pending",
            plano=product.name,
            gateway_id=preference_id
        )
        db.add(new_payment)
        db.commit()
        
        # URL de sucesso simulada (em produção seria o Mercado Pago)
        success_url = f"/sucesso?payment_id={preference_id}&product_id={product_id}"
        
        return {
            "success": True,
            "preference_id": preference_id,
            "init_point": success_url,
            "sandbox_init_point": success_url,
...
async def payment_success(
    ...
    try:
        if payment_id:
            # Buscar pagamento
            payment = db.query(Payment).filter(Payment.gateway_id == payment_id).first()
            if payment:
                ...
                if user and product:
                    ...
                    if user.data_expiracao and user.data_expiracao > datetime.utcnow():
                        ...
                        user.data_expiracao = user.data_expiracao + timedelta(days=product.duration_days)
                    else:
                        # Nova licença
                        user.data_expiracao = datetime.utcnow() + timedelta(days=product.duration_days)
        ...
        return templates.TemplateResponse("success.html", {
            ...
            "message": "Pagamento realizado com sucesso! Sua licença foi ativada."
        })
        
    except Exception as e:
        print(f"Erro ao processar sucesso do pagamento: {e}")
        return templates.TemplateResponse("success.html", {
            "request": request,
            "payment_id": payment_id,
